...
이제 Tomcat 보다는 비교적 간단히 작성된 Intercepting Filter Pattern 구현 예제를 살펴보자. 구현 예제의 실행 또는 테스트 환경이 Tomcat이 아니기 때문에 Tomcat의 요청처리 Architecture인 Valve Pipeline의 컴포넌트 Valve를 FilterChainManager로 대체하고 Filter Chain도 직접 생성하도록 하였다. 마찬가지로, Container 또는 Context의 구현도 필요 없기 때문에 Filter 정보도 FilterChainManager가 직접 관리한다. 그렇게 하면 앞의 Tomcat의 구현보다 훨씬 간결하고 명확하게 Intercepting Filter Pattern 구현을 이해할 수 있다. 다음은 구현 클래스들과 그 관계들을 표현한 다이어그램이다.
