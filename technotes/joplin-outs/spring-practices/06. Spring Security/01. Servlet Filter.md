...
1.  Spring Context 설정
    ...
    ```
    FilterChainManager를 빈 등록 한다. FilterChainManager는 Filter 정보를 관리하기 때문에 Filter 정보를 담은 FilterConfig 객체 배열을 DI 받는다.
...
2.  테스트 setup
 ...
 MockMvc가 빌드 되는 과정으로 앞에서 설정한 Spring Context가 Web Application Context로 등록되고 테스트에는 없지만 Contoller가 발견되면 빈 등록과 함께 요청 핸들러들이 매핑될 것이다 또한 기본 설정으로 DispatcherServlet이 생성되고 설정되는 등등... 기본적인 Spring MVC와 같은 테스트 환경(MockMvc)이 셋업되는 과정이다. 여기서 눈여겨 봐야 하는 것은 DelegatingFilterProxy라는 Filter를 등록하는 부분이다. 실제 Tomcat 환경이라면 web.xml에 DelegatingFilterProxy를 Servlet Filter로 설정하는 것과 동일하다.
 
 재밌는 것은 DelegatingFilterProxy를 생성할 때 생성자를 통해 구현 예제의 FilterChainManager를 Delegator로 세팅하는 것인데 사실, 앞의 예제 클래스 다이어그램에는 표시하지 않았지만 FilterChainManager는 Filter 인터페이스를 구현한 Filter다. FilterChainManager는 Spring Context에 빈으로 등록되어 Servlet Context에 매핑된 Filter(DelegatingFilterProxy)의 특정 요청(/*) 처리를 위임(Deletegate) 받아 요청과 매칭되는 Filter Chain을 생성하고 내부의 Filter를 실행하게 된다.
 
 결론부터 얘기하면, 이 부분은 뒤에서 설명할 Spring Security의 Security Filter Chain이 작동하는 방식과 완전히 동일하다. 
