...
2.  테스트 setup
 ...
 앞의 예제 클래스 다이어그램에는 표시하지 않았지만 FilterChainManager는 Filter 인터페이스를 구현한 Filter로 Spring Context에 빈으로 등록된다. 재미있는 것은 DelegatingFilterProxy가 생성될 때, 생성자를 통해 Delegator로 FilterChainManager가 세팅된다는 사실이다. 따라서 Servlet Context에 등록된 Filter인 DelegatingFilterProxy는 자신이 받은 요청(/*)에 대한 Filter 처리를 FilterChainManager에게 전적으로 위임(Delegate) 하게 되는데, FilterChainManager가 위임받는 Filter 처리는 요청과 매칭되는 Filter들을 가지고 있는 Filter Chain을 생성하고 그 Filter들을 차례로 실행하는 것이다.

 org.springframework.web.filter.DelegatingFilterProxy는 Servlet Context에 등록되어 다른 Context인 Spring Context에 등록된 Filter 빈에게 자신의 이름대로 Filter 처리를 위임(Delegating)하는 Proxy 역할만 하는 Servlet Filter 이다. 예제에 적용된 Proxy Design Pattern을 클래스 다이어그램으로 잠깐 살펴보자.
