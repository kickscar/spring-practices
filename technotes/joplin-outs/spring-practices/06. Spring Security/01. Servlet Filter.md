...
2.  테스트 setup
 ...
 MockMvc가 빌드 되는 과정이다. Web Application Context로는 앞에서 설정한 Spring Context가 등록되고 테스트에는 없지만 Contoller가 발견되면 빈 등록과 함께 요청 핸들러들이 매핑될 것이다. 그리고 기본 설정으로 DispatcherServlet이 생성되고 설정되는 등, 기본적인 Spring MVC와 같은 테스트 환경(MockMvc)이 셋업되는 과정이다. 특히, 눈여겨 봐야 하는 것은 DelegatingFilterProxy 필터 하나를 등록하는 부분이다. 테스트 환경이 아니라면 web.xml에 DelegatingFilterProxy라는 필터를 설정하는 것과 같다.
 
 재미있는 것은 예제의 FilterChainManager를 DelegatingFilterProxy의 생성자를 통해 Delegator로 세팅하면서 Filter를 생성하는 것인데 사실, 구현 예제의 클래스 다이어그램에는 표시하지 않았지만 FilterChainManager는 Filter 인터페이스를 구현한 Filter다. Spring Context에 빈으로 등록되어 Servlet Context에 매핑된 Filter(DelegatingFilterProxy)의 특정 요청(/*) 처리를 위임(Deletegate) 받고 있다.  
