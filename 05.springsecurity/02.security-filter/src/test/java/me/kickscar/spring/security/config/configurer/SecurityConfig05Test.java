...
import me.kickscar.spring.security.config.configurer.SecurityConfig05;
    ...
    public void testSecurityConfiguration(WebApplicationContext context) {
        ...
        /*
            -- Filter Ordering --
             ...
             09. UsernamePasswordAuthenticationFilter               7   : username과 password를 쓰는 form 기반 인증을 처리하는 필터, AuthenticationManager를 통한 인증을 실행, 성공하면 Authentication 객체를 SecurityHolder에 저장한 후 AuthenticationSuccessHandler를 실행, 실패하면 AuthenticationFailureHandler를 실행
             10. DefaultLoginPageGeneratingFilter                   8   : 로그인 기본 페이지를 생성하는 필터
             11. DefaultLogoutPageGeneratingFilter                  9   : 그아웃 기본 페이지를 생성하는 필터
             12. CasAuthenticationFilter                                :
             13. BasicAuthenticationFilter                          10  : HTTP header에 인증 값을 담아 보내는 BASIC 인증을 처리하는 필터
             ...
             21. FilterSecurityInterceptor            [deprecated]  16* : 인가를 결정하는 AccessDicisionManager에게 접근 권한이 있는지 확인하고 처리하는 필터, 앞 필터들을 통과할 때 인증 또는 인가에 문제가 있으면 ExceptionTranslationFilter로 예외를 던진다.
        */
